...
move_t <- function(hR, hC, row, hR1, hC1, col, diag) {
  if(!diag) { return(list("row" = hR1, "col" = hC1)) }
  # ur
...
s <- sapply(2:15, \(i) {
  ...
  if ((sqrt(abs(tC - hC)^2 + abs(tR - hR)^2) > 1)) {
    # print("hello")
    # if(!any(get_adjacent_values(tR, tC, hM) == "H", na.rm = TRUE) | diag) {
    m <- move_t(hR, hC, hR1, hC1, tR, tC, diag)
    tR <<- m$row; tC <<- m$col
...
# get_adjacent_coords(tR, tC, data = tM)[which(get_adjacent_values(tR, tC, tM) == 8),]
tM[1:7,-1:-6]
hM[1:7,-1:-6]
stops |>
  unique() |>
